...
const EvaluateProject = () => {
  ...
  const [projectEvaluation, setProjectEvaluation] = useState("Project Evaluation...");
  ...
  const handleResponse = async () => {
    try {
      ...
      if (responseData.option_1) {
        setOpen(false)
        setWobble(0)
        setProjectEvaluation(responseData.option_1);
      }
    } catch (error) {
  ...
  const handleAnimation = async () => {
    if (projectEvaluation !== "Project Evaluation...") {
      try {
        setProjectEvaluation("");
          ...
          <div className="flex gap-7 flex-wrap">
            <div className="p-7 sm:max-w-[848px] w-full h-[350px] shadow-card rounded-2xl"
              ...
              <div className="text-xl text-[#666]" style={{ scrollBehavior: "auto" }}>
                <ReactMarkdown children={projectEvaluation} />
              </div>
            </div>
