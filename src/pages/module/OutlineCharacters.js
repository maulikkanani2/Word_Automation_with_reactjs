...
import { mdiArrowRightBold, mdiChevronDown, mdiDotsVertical, mdiRefresh } from "@mdi/js";
...
const OutlineCharacters = () => {
  const [selectFilterVal, setSelectFilterVal] = useState("Novel/Script");
  const [plot_1, setPlot_1] = useState("some Idea........");
  const [plot_2, setPlot_2] = useState("some Idea........");
  ...
  const handleResponse = async () => {
    try {
      ...
      setWobble(1)
      setOpen(true)
      const responseData = await fetchData('develop_characters/', JSON.stringify(payload));
      ...
      if (responseData.option_1 && responseData.option_2) {
        setWobble(0)
        setOpen(false)
        setPlot_1(responseData.option_1);
        setPlot_2(responseData.option_2);
      }
    } catch (error) {
  ...
  const handleAnimation = async () => {
    if (plot_1 !== "some Idea........" && plot_2 !== "some Idea........") {
      try {
        setPlot_1("");
        setPlot_2("");
    ...
    <PageLayout header={header}>
      <div>
        ...
        <div className="flex gap-7 flex-wrap">
          <div className="p-7 sm:max-w-[330px] w-full h-[350px] shadow-card rounded-2xl" style={{ overflow: "auto" }}>
          ...
          <div className="p-7 sm:max-w-[330px] w-full h-[350px] shadow-card rounded-2xl" style={{ overflow: "auto" }}>
            <div className="flex items-center justify-between mb-7">
              <div className="text-2xl font-semibold">Plot 2</div>
              <Icon
                path={mdiDotsVertical}
                size={"28px"}
                className="cursor-pointer"
