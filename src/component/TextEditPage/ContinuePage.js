...
const ContinuePage = (props) => {
  ...
  const [continueResponse, setContinueResponse] = useState("Text.....");
  ...
  const insert = () => {
    props?.setResponseData(continueResponse)
    props?.setOpen(false)
  }
  ...
  const payload = { "input_text": input_text, "story_type": selectFilterVal}
  const handleResponse = async () => {
    try {
      setContinueResponse("");
      setWobble(1)
      setOpen(true)
      const responseData = await fetchData('continue/', JSON.stringify(payload));
      ...
      if (responseData.option_1) {
        setWobble(0)
        setOpen(false)
        setContinueResponse(responseData.option_1);
      }
    } catch (error) {
        ...
        <div className="flex gap-7 flex-wrap">
          <div className="p-7 sm:max-w-[848px] w-full h-[350px] shadow-card rounded-2xl" style={{ overflow: "auto" }}>
            ...
            <div className="text-xl text-[#666]" style={{ scrollBehavior: "auto" }}>
              <ReactMarkdown children={continueResponse} />
            </div>
          </div>
